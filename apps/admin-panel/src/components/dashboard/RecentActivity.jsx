                    ...
                    <div key={item.id} className="flex items-start gap-4 group cursor-pointer">
                        ...
                        <div className="flex-1">
                            <div className="flex justify-between items-start">
                                <div>
                                    <h4 className="text-sm font-semibold text-gray-800 dark:text-gray-200 group-hover:text-blue-600 transition">{item.restaurant}</h4>
                                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{item.action}</p>
                                </div>
                                <span className={`text-xs font-medium px-2 py-1 rounded-md ${item.amount.startsWith('+') ? 'bg-green-50 text-green-600 dark:bg-green-900/20' : 'text-gray-400'
                                    }`}>
                                    {item.amount}
                                </span>
                            </div>
